...
class TestDiceRandomBinaryClassificationMethods:
    ...
    def test_invalid_query_instance(self, desired_range, desired_class, sample_custom_query_1, total_CFs,
        ...
        # self.exp.dataset_with_predictions, self.exp.random_tree, self.exp.predictions = \
        #     self.exp.build_random_tree(self.data_df_copy, desired_range, desired_class, self.exp.predicted_outcome_name)
    ...
    def test_random_counterfactual_explanations_output(self, desired_class, sample_custom_query_1, total_CFs):
        ...
        assert all(counterfactual_explanations.cf_examples_list[0].final_cfs_df.Numerical == expected_output.Numerical[0]) and \
               all(counterfactual_explanations.cf_examples_list[0].final_cfs_df.Categorical == expected_output.Categorical[0])
...
class TestDiceRandomMultiClassificationMethods:
    ...
    # Testing that the output of multiclass classification lies in the desired_class
    @pytest.mark.parametrize("desired_class, total_CFs", [(2, 3)])
    def test_random_tree_counterfactual_explanations_output(self, desired_class, sample_custom_query_2, total_CFs):
...
class TestDiceRandomRegressionMethods:
    ...
    def test_random_output(self, desired_range, sample_custom_query_2, total_CFs):
