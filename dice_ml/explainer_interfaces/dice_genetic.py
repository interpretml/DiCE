...
class DiceGenetic(ExplainerBase):
    ...
    def update_hyperparameters(
        self, proximity_weight, sparsity_weight, diversity_weight, categorical_penalty
    ):
    ...
    def do_loss_initializations(
        self, yloss_type, diversity_loss_type, feature_weights, encoding="one-hot"
    ):
            ...
            if encoding == "one-hot":
                ...
                for feature in self.data_interface.feature_names:
                    if feature in feature_weights:
                        ...
                        feature_weights_list.append(
                            round(1 / self.feature_range[feature].max(), 2)
                        )
            self.feature_weights_list = [feature_weights_list]

    def do_random_init(
        self, num_inits, features_to_vary, query_instance, desired_class, desired_range
    ):
    ...
    def do_KD_init(
        self, features_to_vary, query_instance, cfs, desired_class, desired_range
    ):
        cfs = self.label_encode(cfs)
        cfs = cfs.reset_index(drop=True)

        self.cfs = np.zeros(
            (self.population_size, self.data_interface.number_of_features)
        )
        ...
        if len(uniques) != self.population_size:
            remaining_cfs = self.do_random_init(
                self.population_size - len(uniques),
                features_to_vary,
                query_instance,
                desired_class,
                desired_range,
    ...
    def do_cf_initializations(
        ...
        if initialization == "random":
            ...
            self.dataset_with_predictions, self.KD_tree, self.predictions = (
                self.build_KD_tree(
                    self.data_interface.data_df.copy(),
                    desired_range,
                    desired_class,
            ...
            if self.KD_tree is None:
                ...
                num_queries = min(
                    len(self.dataset_with_predictions),
                    self.population_size * self.total_CFs,
                )
                indices = self.KD_tree.query(query_instance_df_dummies, num_queries)[1][
                    0
                ]
    ...
    def do_param_initializations(
        ...
        self.do_loss_initializations(
            yloss_type, diversity_loss_type, feature_weights, encoding="label"
        )
    ...
    def _generate_counterfactuals(
        ...
        desired_class = self.misc_init(
            stopping_threshold, desired_class, desired_range, test_pred[0]
        )
        ...
        query_instance_df = self.find_counterfactuals(
            query_instance,
            desired_range,
            desired_class,
            features_to_vary,
        ...
        desired_class_param = (
            self.decode_model_output(pd.Series(self.target_cf_class[0]))[0]
            if hasattr(self, "target_cf_class")
            else desired_class
        )
    ...
    def _predict_fn_custom(self, input_instance, desired_class):
        ...
        if (
            self.model.model_type == ModelTypes.Classifier
            and np.array(output).shape[1] == 1
        ):
            # DL models return only 1 for binary classification
            ...
            if self.yloss_type == "hinge_loss":
                ...
                for i in range(len(predicted_value)):
                    if not desired_range[0] <= predicted_value[i] <= desired_range[1]:
                        yloss[i] = min(
                            abs(predicted_value[i] - desired_range[0]),
    ...
    def compute_proximity_loss(self, x_hat_unnormalized, query_instance_normalized):
        ...
        feature_weights = np.array(
            [
                self.feature_weights_list[0][i]
                for i in self.data_interface.continuous_feature_indexes
            ]
        )
        product = np.multiply(
            (
                abs(x_hat - query_instance_normalized)[
                    :, [self.data_interface.continuous_feature_indexes]
                ]
            ),
            feature_weights,
        )
    ...
    def compute_sparsity_loss(self, cfs):
        ...
        return sparsity_loss / len(
            self.data_interface.feature_names
        )  # Dividing by the number of features to normalize sparsity loss
    ...
    def compute_loss(self, cfs, desired_range, desired_class):
        ...
        self.sparsity_loss = (
            self.compute_sparsity_loss(cfs) if self.sparsity_weight > 0 else 0.0
        )
        self.loss = np.reshape(
            np.array(
                self.yloss
    ...
    def find_counterfactuals(
        ...
        while iterations < maxiterations and self.total_CFs > 0:
            if abs(previous_best_loss - current_best_loss) <= thresh and (
                self.model.model_type == ModelTypes.Classifier
                and all(i == desired_class for i in cfs_preds)
                or (
                    self.model.model_type == ModelTypes.Regressor
                    and all(
                        desired_range[0] <= i <= desired_range[1] for i in cfs_preds
                    )
                )
            ):
                stop_cnt += 1
            ...
            population_fitness = self.compute_loss(
                population, desired_range, desired_class
            )
            ...
            new_generation_1 = np.array(
                [population[int(tup[0])] for tup in population_fitness[:top_members]]
            )
            ...
            if rest_members > 0:
                new_generation_2 = np.zeros(
                    (rest_members, self.data_interface.number_of_features)
                )
                for new_gen_idx in range(rest_members):
                    population_indices = list(range(int(len(population) / 2)))
                    parent1_idx = random.choice(population_indices)
                    parent2_idx = random.choice(population_indices)
                    parent1 = population[parent1_idx]
                    parent2 = population[parent2_idx]
                    child = self.mate(
                        parent1, parent2, features_to_vary, query_instance
                    )
            ...
            if new_generation_2 is not None:
                ...
                raise SystemError(
                    "The number of total_Cfs is greater than the population size!"
                )
            iterations += 1
        ...
        while i < self.total_CFs:
            ...
            if self.is_cf_valid(predictions):
                ...
                if (
                    not isinstance(predictions, (np.floating, float))
        ...
        query_instance_df[self.data_interface.outcome_name] = (
            self.get_model_output_from_scores(self.test_pred)
        )
        ...
        query_instance_df, self.final_cfs_df, self.final_cfs_df_sparse = (
            self.decode_to_original_labels(
                query_instance_df, self.final_cfs_df, self.final_cfs_df_sparse
            )
        )
        ...
        if verbose:
            if len(self.final_cfs) == self.total_CFs:
                print(
                    "Diverse Counterfactuals found! total time taken: %02d" % m,
                    "min %02d" % s,
                    "sec",
                )
            else:
                print(
                    "Only %d (required %d) " % (len(self.final_cfs), self.total_CFs),
                    "Diverse Counterfactuals found for the given configuation, perhaps ",
                    "change the query instance or the features to vary..."
                    "; total time taken: %02d" % m,
                    "min %02d" % s,
                    "sec",
                )
    ...
    def label_encode(self, input_instance):
        for column in self.data_interface.categorical_feature_names:
            input_instance[column] = self.labelencoder[column].transform(
                input_instance[column]
            )
    ...
    def label_decode(self, labelled_input):
        ...
        for j in range(num_to_decode):
            ...
            for i in range(len(labelled_input[j])):
                if (
                    self.data_interface.feature_names[i]
                    in self.data_interface.categorical_feature_names
                ):
                    enc = self.labelencoder[self.data_interface.feature_names[i]]
                    val = enc.inverse_transform(
                        np.array([labelled_input[j][i]], dtype=np.int32)
                    )
        ...
        input_instance_df = pd.DataFrame(
            input_instance, columns=self.data_interface.feature_names
        )
    ...
    def get_valid_feature_range(self, normalized=False):
        ret = self.data_interface.get_valid_feature_range(
            self.feature_range, normalized=normalized
        )
