...
class DiceTensorFlow2Multi(ExplainerBase):
    ...
    def __init__(self, data_interface, model_interface):
        ...
        print("* DiceTensorFlow2Multi initiated")
    ...
    def generate_counterfactuals(self, query_instance, total_CFs, desired_class="opposite", proximity_weight=0.5,
        ...
        :param desired_class: Desired counterfactual class (target class) - takes an int.
        ...
        # Modified
        if type(desired_class) is not int:
            raise ValueError("desired_class should be an int.")
    ...
    def predict_fn(self, input_instance):
        ...
        # Modified
        return np.array([preds for preds in temp_preds], dtype=np.float32)
    ...
    def compute_yloss(self):
        ...
        for i in range(self.total_CFs):
            if self.yloss_type == "l2_loss":
                ...
                temp_loss = temp_loss[:, self.desired_class:(self.desired_class + 1)][0][0]  # Modified
                ...
                temp_logits = temp_logits[:, self.desired_class:(self.desired_class + 1)]  # Modified
    ...
    def stop_loop(self, itr, loss_diff):
        ...
        if loss_diff <= self.loss_diff_thres:
            ...
            if self.loss_converge_iter < self.loss_converge_maxiter:
                ...
                # Modified
                temp_preds = [self.predict_fn(tf.constant(cf, dtype=tf.float32)) for cf in temp_cfs]
                test_preds = [p[0, self.desired_class] for p in temp_preds]
                class_preds = [np.argmax(p, axis=1)[0] for p in temp_preds]

                if all(i == self.desired_class for i in class_preds) and all(i >= self.stopping_threshold for i in test_preds):
                    self.converged = True
                    return True
                else:
                    return False
    ...
    def find_counterfactuals(self, query_instance, desired_class, optimizer, learning_rate, min_iter,
        ...
        self.target_cf_class = np.array([[self.desired_class]], dtype=np.float32)
        ...
        self.best_backup_cfs_class = [0]*max(self.total_CFs, loop_find_CFs)
        ...
        for loop_ix in range(loop_find_CFs):
            ...
            while self.stop_loop(iterations, loss_diff) is False:
                ...
                # Modified
                temp_preds = [self.predict_fn(tf.constant(cf, dtype=tf.float32)) for cf in temp_cfs_stored]
                test_preds_stored = [p[0, self.desired_class] for p in temp_preds]
                class_preds_stored = [np.argmax(p, axis=1)[0] for p in temp_preds]

                if all(i == self.desired_class for i in class_preds_stored) and all(i >= self.stopping_threshold for i in test_preds_stored):
                    ...
                    if avg_preds_dist < self.min_dist_from_threshold[loop_ix]:
                        ...
                        for ix in range(self.total_CFs):
                            ...
                            self.best_backup_cfs_class[loop_ix+ix] = copy.deepcopy(class_preds_stored[ix])
        ...
        # Modified
        temp_preds = [self.predict_fn(cfs) for cfs in self.final_cfs]
        self.cfs_preds = [p[0, self.desired_class] for p in temp_preds]
        self.class_preds = [np.argmax(p, axis=1)[0] for p in temp_preds]

        # update final_cfs from backed up CFs if valid CFs are not found
        if any(i != self.desired_class for i in self.class_preds) or any(i < self.stopping_threshold for i in self.cfs_preds):
            for loop_ix in range(loop_find_CFs):
                if self.min_dist_from_threshold[loop_ix] != 100:
                    for ix in range(self.total_CFs):
                        ...
                        self.class_preds[loop_ix + ix] = copy.deepcopy(self.best_backup_cfs_class[loop_ix + ix])
        ...
        # Modified
        if all(i == self.desired_class for i in self.class_preds) and all(i >= self.stopping_threshold for i in self.cfs_preds):
            ...
            for cf_ix, pred in enumerate(self.cfs_preds):
                if self.class_preds[cf_ix] == self.desired_class and pred >= self.stopping_threshold:
    ...
    # Modified: override the is_valid cf function from the ExplainerBase
    def is_cf_valid(self, model_score):
        """Check if a cf belongs to the target class or target range.
        """
        validity = True if np.argmax(model_score) == self.desired_class and model_score[0, self.desired_class] >= self.stopping_threshold else False
